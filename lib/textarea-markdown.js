...
var TextareaMarkdown = function () {
  ...
  _createClass(TextareaMarkdown, [{
    ...
    value: async function upload(file) {
      ...
      reader.onload = async function () {
        ...
        try {
          ...
          var dimensions = await _this4.getImageSize(url);
          var uploadTag = _this4.replacePlaceholderTag(placeholderTag, file.name, fileSize, url, dimensions);
    ...
    value: function replacePlaceholderTag(placeholderTag, filename, fileSize, url, dimensions) {
      if (placeholderTag !== this.options.uploadOtherTag) {
        return placeholderTag.replace(/%filename/, filename).replace(/%url/, url).replace(/%width/, dimensions.width).replace(/%height/, dimensions.height);
      } else {
        return placeholderTag.replace(/%filename/, filename).replace(/%url/, url).replace(/%fileSize/, fileSize).replace(/%width/, dimensions.width).replace(/%height/, dimensions.height);
      }
    }
  }, {
    key: "getImageSize",
